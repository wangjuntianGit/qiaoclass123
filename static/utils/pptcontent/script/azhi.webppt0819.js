	...
	this.addShape = function(opt){
		...
		switch(shape.type){
			...
			case 'RichText':
				...
				if(shape.geomWidth && shape.geomHeight)
					richTextBoxSvg.attr('viewBox', '0 0 ' + shape.geomWidth + ' ' + shape.geomHeight)
				...
				switch(shape.geom){
					...
					case 'downArrow':
						...
						points.push((shape.width / scale) + ','
							+ ((shape.height - Math.sqrt(Math.pow(shape.width, 2) - Math.pow(shape.width / 2, 2)) * .6) / scale));
						points.push((shape.width / 2 / scale) + ',' + (shape.height / scale));
						points.push(0 + ','	+ ((shape.height - Math.sqrt(Math.pow(shape.width, 2) - Math.pow(shape.width / 2, 2)) * .6) / scale));
						points.push((shape.width * .25 / scale) + ',' + ((shape.height - Math.sqrt(Math.pow(shape.width, 2) - Math.pow(shape.width / 2, 2)) * .6) / scale));
						points.push((shape.width * .25 / scale) + ',' + 0);
						points.push((shape.width * .75 / scale) + ',' + 0);
						points.push((shape.width * .75 / scale) + ',' + ((shape.height - Math.sqrt(Math.pow(shape.width, 2) - Math.pow(shape.width / 2, 2)) * .6) / scale));
					...
					case 'cust':
						...
						for (var x=0; x<shape.custGeom.length; x++){
							...
							for(var xx=0; xx<shape.custGeom[x].points.length; xx++)
								s += shape.custGeom[x].points[xx][0] + ' ' + shape.custGeom[x].points[xx][1] + ' ';
...
*/
	...
	this.getShapeText = function(opt){
		...
		if(richText.fontSubScript) fontSize = fontSize *.6;
		...
		var text = '<span style="' +
				...
				(richText.fontSubScript ? 'margin-top:'+(fontSize/.6*.4)+'px; ' : '') +
...
//					console.log(borderImage)
		...
		if(shape.horizontal)
			dom.css('justify-content', this.getHorizontal(shape.horizontal));
		...
		if(shape.placeholderType){
			...
			for(var i in _data.masterSlides){
				for(var j in _data.masterSlides[i].shape){
					...
					if(shape.placeholderType==masterShape.placeholderType)
						break;
				}			
			}
			...
			if(masterShape){
				...
				if(dom.css('justify-content')=='')
					dom.css('justify-content', this.getHorizontal(listStyle.align));

				if(masterShape.vertical)
					dom.css('align-items', this.getVertical(masterShape.vertical));
			}
		}
